...
def parse_url_with_scrapingbee(url: str) -> str:
    ...
    if not SCRAPINGBEE_API_KEY:
        raise ValueError(
            ...
            "Передайте его как переменную окружения SCRAPINGBEE_API_KEY\n"
            "(например, через docker run -e SCRAPINGBEE_API_KEY=ваш_ключ ...)."
        )
    ...
    params = {
        ...
        "premium_proxy": "true",       # Резидентские прокси
        "country_code": "ru",          # Российские IP (при необходимости можно поменять)
    ...
    try:
        ...
        logger.info(f"ScrapingBee статус: {response.status_code}")
        ...
        if response.status_code == 200:
            ...
            logger.error("❌ Доступ запрещен (проверьте API ключ ScrapingBee)")
            raise PermissionError("Неверный API ключ ScrapingBee")

        elif response.status_code == 429:
            logger.error("❌ Превышен лимит запросов ScrapingBee")
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if message.document and message.document.mime_type == "application/pdf":
            ...
            # Сначала проверяем, похоже ли сообщение на ссылку
...
def main() -> None:
    ...
    if not token:
        ...
        print("Передайте TELEGRAM_BOT_TOKEN как переменную окружения")
        print("Например: docker run -e TELEGRAM_BOT_TOKEN=... ...")
        return

    if not openai_key:
        print("⚠️ ВНИМАНИЕ: OPENAI_API_KEY не найден")
        print("AI функции не будут работать.")
        print("Передайте OPENAI_API_KEY как переменную окружения.")

    if not scrapingbee_key:
        print("⚠️ ВНИМАНИЕ: SCRAPINGBEE_API_KEY не найден")
        print("Парсинг ссылок через ScrapingBee не будет работать.")
        print("Передайте SCRAPINGBEE_API_KEY как переменную окружения.")
